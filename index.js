...
// Rakit pesan rundown final dengan format yang lebih cantik
function formatRundownMessage(lokasi, data) {
    let msg = `╔══════════════════════╗\n`;
    msg += `   🗺️ *RENCANA PERJALANAN*\n`;
    msg += `   📍 ${titleCase(lokasi)}\n`;
    msg += `╚══════════════════════╝\n\n`;

    // Rekomendasi Tempat dengan emoji yang lebih menarik
    msg += `┌─ 📍 *TEMPAT PILIHAN* ─────────\n`;
    (data.rekomendasi || []).forEach((t, i) => { 
        const emoji = ['🎯', '🎪', '🎨', '🎭', '🎪', '🎯', '🎨', '🎭', '🎪', '🎯'][i % 10];
        msg += `│ ${emoji} ${t}\n`; 
    });
    msg += `└──────────────────────────\n\n`;

    // Rundown dengan format timeline yang lebih visual
    msg += `┌─ 📝 *JADWAL KEGIATAN* ────────\n`;
    (data.rundown || []).forEach((item, idx) => {
        msg += `│\n`;
        msg += `│ ⏰ *${item.waktu}*\n`;
        if (item.tempat) {
            msg += `│ 📍 _${item.tempat}_\n`;
        }
        (item.aktivitas || []).forEach(a => { 
            msg += `│    • ${a}\n`; 
        });
    });
    msg += `│\n`;
    ...
    // Cuaca dengan format yang lebih informatif
    msg += `┌─ 🌤️ *INFO CUACA* ────────────\n`;
    (data.cuaca || []).forEach(c => {
        const weatherEmoji = getWeatherEmoji(c.kondisi);
        msg += `│ ${weatherEmoji} ${c.jam} — ${c.kondisi}\n`;
        msg += `│    🌡️ ${c.suhu}°C  💧 ${c.hujan}%\n`;
    });
    msg += `└──────────────────────────\n`;
    ...
    msg += `   💬 *AKSI LANJUTAN*\n`;
    msg += `╠══════════════════════╣\n`;
    msg += `   • Ketik *rombak* → atur ulang jadwal\n`;
    msg += `   • Ketik *lain* → ganti pilihan tempat\n`;
    msg += `   • Ketik *oke* → mulai perjalanan!\n`;
    msg += `╚══════════════════════╝`;

    return msg;
}

// Helper untuk emoji cuaca
...
function kirimDaftarTempat(ctx, lokasi, daftarTempat) {
    ...
    msg += `\n┌─ 📋 *CARA PILIH* ────────────\n`;
    msg += `│  Balas nomor tempatnya ya!\n`;
    msg += `│  Bisa lebih dari satu: *1 3 4*\n`;
    msg += `│  Atau cukup satu: *1*\n`;
    msg += `│\n`;
    msg += `│  🔄 Ketik *lain* → ganti semua pilihan\n`;
    msg += `└──────────────────────────`;
